type Props = {
  onEnterDetails: () => void;
  onScanCoverAgain: () => void;
  onCancel: () => void;
};

export default function CoverNotRecognizedPage({ onEnterDetails, onScanCoverAgain, onCancel }: Props) {
  return (
    <div className="fixed inset-0 flex flex-col overflow-y-auto p-lg lg:flex-row lg:items-start lg:gap-lg lg:overflow-hidden">
      <p className="shrink-0 text-lg font-medium text-ink-primary lg:hidden">Result — Cover Not Recognized</p>

      <div className="mt-md flex aspect-[3/4] w-full shrink-0 items-center justify-center overflow-hidden rounded-md border border-line bg-surface-subtle lg:mt-0 lg:aspect-auto lg:h-[400px] lg:w-[300px]">
        <p className="p-lg text-center text-sm text-ink-muted">No match found</p>
      </div>
      ...
      <div className="flex flex-1 flex-col">
        <span className="mt-lg inline-flex w-fit items-center rounded-full border border-amber-200 bg-amber-50 px-md py-xs text-xs font-medium text-amber-800 lg:mt-0">
          COVER NOT RECOGNIZED
        </span>

        <h1 className="mt-lg text-2xl font-semibold text-ink-primary">We couldn&rsquo;t identify this book</h1>
        <p className="mt-sm text-base text-ink-muted">
          We couldn&rsquo;t match this book from its cover or its barcode. You can enter the title and author manually.
        </p>
